...
export class ScriptLoaderError extends Error {
  ...
  constructor(message: string, path: string, stack: string[] = [], line?: number, position = 0) {
    super(message);
...
const isPossiblyMappedPath = (path: string) => path && ['~', '<'].includes(path[0]);
...
export class ScriptLoader {
  ...
  private clientScripts = new WeakMap<RedisClientInstance, Set<string>>();
  /**
  ...
  private async resolveDependencies(
    ...
    while ((res = IncludeRegex.exec(content)) !== null) {
      ...
      includePaths = includePaths.filter((file: string) => path.extname(file) === '.lua');
      ...
      for (let i = 0; i < includePaths.length; i++) {
        ...
        const hasInclude = file.includes.find((x: ScriptMetadata) => x.path === includePath);
        ...
        if (!includeMetadata) {
          ...
          let childContent = '';
          try {
            const buf = readFile(includePath, { flag: 'r' });
  ...
  async loadCommand(filename: string, cache?: Map<string, ScriptMetadata>): Promise<Command> {
    filename = path.resolve(filename);

    const { name: scriptName } = splitFilename(filename);
    let script = cache?.get(scriptName);
    if (!script) {
      const content = readFile(filename).toString();
   ...
   *
   */
  async loadScripts(dir?: string, cache?: Map<string, ScriptMetadata>): Promise<Command[]> {
    ...
    const luaFiles = files.filter((file: string) => path.extname(file) === '.lua');
